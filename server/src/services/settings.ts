...
export async function loadSettings(): Promise<Settings> {
  ...
  try {
    ...
    // `passwordHash` as the override password and `userPasswordHash` as the login
    // password (empty = the default 123456). So that an old file can't be
    // backdoored with 123456, move the old password over to `userPasswordHash`
    // and then clear the override field.
